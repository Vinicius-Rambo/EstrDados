...
Lista insereLista(Lista l, int e) {
    Lista novo = (Lista)malloc(sizeof(struct elemento)); // Aloca o espaço e faz as atribuições de valores
    novo->dado = e;
    novo->prox = NULL;
    ...
    // Procura o ponto de inserido na lista
    while ((p != NULL) && (p->dado < e)) {
        ant = p;
        p = p->prox;
    }

    if (ant == NULL) { // Inserindo no inicio
        novo->prox = l;
        l = novo;
